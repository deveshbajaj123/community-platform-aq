...
import { timeUntil, pad, PARADOX_TARGET } from '../lib/utils'
...
function Hero() {
  ...
  useEffect(() => {
    const tick = () => setT(timeUntil(PARADOX_TARGET))
  ...
  return (
    <section className="relative overflow-hidden border-b-[1.5px] border-ink" style={{ minHeight: 'calc(100dvh - 72px)', borderTop: '4px solid var(--c1)' }}>
      ...
      {/* Badges — always visible (xs breakpoint doesn't exist; was always hidden before) */}
      <div className="absolute top-4 right-4 sm:top-5 sm:right-6 z-10 flex gap-2.5 items-start">
        <motion.div
          initial={{ scale: 0, rotate: -12 }} animate={{ scale: 1, rotate: -5 }}
          transition={{ type: 'spring', stiffness: 220, damping: 18, delay: 0.55 }}
          className="flex flex-col items-center justify-center rounded-full border-[1.5px] border-ink"
          style={{ width: 68, height: 68, background: 'var(--c2)', filter: 'drop-shadow(3px 3px 0 var(--ink))' }}
        >
          <span className="font-display text-[32px] leading-[0.95] text-ink" style={{ letterSpacing: '-0.02em' }}>5</span>
          <span className="font-mono text-[7px] tracking-[0.08em] uppercase text-ink/70 mt-0.5">years of aq</span>
        </motion.div>
        <motion.div
          initial={{ scale: 0, rotate: 10 }} animate={{ scale: 1, rotate: 4 }}
          transition={{ type: 'spring', stiffness: 220, damping: 18, delay: 0.7 }}
          className="flex flex-col items-center rounded-lg border-[1.5px] border-ink px-2.5 py-1.5 mt-3"
          style={{ background: 'var(--c1)', color: 'var(--bg)', filter: 'drop-shadow(3px 3px 0 var(--ink))' }}
        >
          <span className="font-display text-[24px] leading-[0.95]">04</span>
          <span className="font-mono text-[7px] tracking-[0.06em] opacity-85">th edition</span>
        </motion.div>
      </div>
        ...
        >
          <span className="relative flex h-2 w-2 shrink-0">
            <motion.span animate={{ scale: [1, 2.2, 1], opacity: [0.7, 0, 0.7] }}
              ...
              className="absolute inset-0 rounded-full" style={{ background: 'var(--c1)' }}/>
            <span className="relative rounded-full h-2 w-2" style={{ background: 'var(--c1)' }}/>
          </span>
          <span className="font-mono text-[11px] tracking-[0.16em] uppercase opacity-65">
            kolkata · jun 3–7 · all proceeds to charity
          </span>
        </motion.div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-end gap-6 lg:gap-14 mt-2">
          ...
          >
            ...
            <p className="font-body text-[17px] sm:text-[19px] leading-[1.4] opacity-85 m-0">
              kolkata's biggest inter-school fest.
            </p>
            {/* Typewriter cycling line */}
            <p className="font-mono text-[13px] sm:text-[14px] leading-[1.6] mt-2 m-0"
              style={{ color: 'var(--c2)', minHeight: '1.6em' }}>
              <Typewriter
                text={[
                  '12+ events, 5 days.',
                  '₹51,000 in prizes.',
                  'all proceeds to charity.',
                  '800+ students competing.',
                  'jun 3–7 · kolkata.',
                  '100% to welfare drives.',
                  'register free. show up.',
            ...
            <div className="flex gap-3 mt-5 flex-wrap">
              <MotionLink to="/paradox/register"
              ...
              <MotionLink to="/paradox/events"
                ...
                style={{ minHeight: 44, willChange: 'background-color' }}>
                see the lineup
              </MotionLink>
            </div>
          </motion.div>
          ...
          >
            <p className="font-mono text-[10px] tracking-[0.14em] uppercase opacity-50 mb-2">countdown to jun 3</p>
            <div className="grid grid-cols-4 gap-1.5 sm:gap-2 max-w-[340px] lg:max-w-[400px]">
              {([
                { v: t.d, label: 'days',    style: { background: 'var(--ink)', color: 'var(--bg)' } },
                { v: t.h, label: 'hrs',     style: { background: 'var(--c1)', color: 'var(--bg)' } },
                { v: t.m, label: 'min',     style: { background: 'var(--c2)', color: 'var(--ink)' } },
                { v: t.s, label: 'sec',     style: { background: 'var(--c3)', color: 'var(--ink)' } },
              ] as const).map(({ v, label, style }) => (
                <div key={label} className="rounded-xl text-center p-2.5 sm:p-4 border-[1.5px] border-ink" style={{ ...style, boxShadow: '3px 3px 0 var(--ink)' }}>
                  <div className="relative overflow-hidden flex items-center justify-center" style={{ height: 52 }}>
                    <AnimatePresence mode="popLayout" initial={false}>
                      <motion.span key={pad(v)}
                        initial={{ y: -44, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 44, opacity: 0 }}
                        transition={{ type: 'spring', stiffness: 420, damping: 32 }}
                        className="font-display block"
                        style={{ fontSize: 'clamp(26px, 5.5vw, 40px)', lineHeight: 1.15, fontVariantNumeric: 'tabular-nums' }}>
                        {pad(v)}
                      </motion.span>
                    </AnimatePresence>
                  </div>
                  <div className="font-mono text-[9px] sm:text-[10px] tracking-[0.12em] uppercase opacity-65 mt-1">
                    {label}
                  </div>
                </div>
              ))}
...
// ─── MARQUEE ───────────────────────────────────────────────────────────────
const MARQUEE_ITEMS = [
  'PARADOX 2026', '★', 'JUN 3–7', '★', 'KOLKATA', '★', '5 YEARS OF AQ', '★',
  '12+ EVENTS', '★', '100% TO CHARITY', '★', 'REGISTER NOW', '★',
]
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 relative" style={{ zIndex: 2 }}>
        ...
        <div className="grid gap-8 lg:gap-14 lg:grid-cols-[1.2fr_1fr] lg:items-center">
          ...
          <motion.p className="font-body leading-relaxed opacity-90 m-0"
            ...
            paradox is aquaterra's flagship inter-school + college week — sport, business, creative, cultural events stitched together by one big after-party. every rupee funds welfare projects. you show up, you compete, kids on the other side of the city eat &amp; learn.
          </motion.p>
          ...
          >
            {[
              { n: '800+', l: 'expected footfall', bg: 'var(--c1)', color: 'var(--bg)' },
              { n: '12+',  l: 'events',             bg: 'var(--c2)', color: 'var(--ink)' },
              { n: '7',    l: 'days',                bg: 'var(--c3)', color: 'var(--ink)' },
              { n: '100%', l: 'to charity',          bg: 'var(--ink)', color: 'var(--bg)' },
            ].map((s) => (
  ...
  return (
    <section className="border-b-[1.5px] border-ink relative overflow-hidden pt-6 pb-7 sm:pt-14 sm:pb-16" style={{ position: 'relative', zIndex: 1 }}>
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 relative" style={{ zIndex: 2 }}>
        ...
        <MotionLink to="/paradox/events" whileTap={{ scale: 0.97 }} transition={SPRING}
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 grid gap-10 lg:grid-cols-[1.3fr_1fr] lg:items-center relative" style={{ zIndex: 2 }}>
        ...
        <motion.div variants={stagger(0.07)} initial="hidden" whileInView="show" viewport={{ once: true, margin: '-50px' }}>
          ...
          <motion.p variants={revealLeft} className="font-body leading-relaxed opacity-80 m-0"
            ...
            AquaTerra is a student-led NGO run by 1,100+ Kolkata kids. Five years in, we've raised ₹10L+ across 550+ welfare projects — teaching workshops, worksheets, food, plantation, animal welfare. paradox is our biggest single-week fundraiser.
          </motion.p>
          ...
          <motion.div variants={revealLeft} className="flex flex-wrap gap-2 mt-5">
            {[
              ...
              { l: '★ 800+ volunteers', bg: 'var(--bg)', color: 'var(--ink)' },
            ].map(tag => (
          ...
          <motion.div variants={revealLeft}>
            <MotionLink to="/paradox/sponsor"
              whileHover={{ x: -1, y: -1 }} whileTap={{ scale: 0.97 }} transition={SPRING}
    ...
    <section className="border-b-[1.5px] border-ink relative overflow-hidden pt-6 pb-7 sm:pt-14 sm:pb-16"
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 relative" style={{ zIndex: 2 }}>
        ...
        <motion.h2 className="font-display leading-[0.9] text-balance mt-1.5 mb-4 sm:mb-8"
          ...
          viewport={{ once: true }}
          transition={{ type: 'spring', stiffness: 220, damping: 24, delay: 0.08 }}>
          four editions deep. <span style={{ color: 'var(--c1)' }}>five years of aq.</span>
        </motion.h2>

        {/* Fix 7: popIn variant with stagger, P2026 card gets whileHover scale:1.05 */}
        <motion.div variants={stagger(0.07)} initial="hidden" whileInView="show"
          ...
          {PAST.map((p) => (
            <motion.div key={p.tag} variants={popIn}
              whileHover={p.scale ? { y: -5, scale: 1.05 } : { y: -5 }} transition={SPRING}
              className="rounded-2xl border-[1.5px] border-ink p-5 flex flex-col gap-2"
              style={{
              ...
              <span className="font-mono text-[11px] tracking-[0.1em] uppercase opacity-85">{p.tag}</span>
              <span className="font-display leading-[0.95]" style={{ fontSize: 32, fontVariantNumeric: 'tabular-nums' }}>{p.year}</span>
              <span className="font-display leading-[1.05] opacity-95" style={{ fontSize: 17 }}>{p.stat}</span>
              <span className="font-body leading-relaxed opacity-82 text-[13px] mt-auto">{p.body}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
...
// ─── SPONSORS ──────────────────────────────────────────────────────────────
const TIERS = [
  { name: 'Title',             amt: '₹70K', perks: 'standees · 30+ stories · chief guest · 3 IG posts · stall · samples', bg: 'var(--ink)', color: 'var(--bg)', amtColor: 'var(--c2)' },
  { name: 'Co-Sponsor',        amt: '₹50K', perks: 'standees · 20+ stories · IG post · reel · stall · samples', bg: 'var(--c1)', color: 'var(--bg)', amtColor: 'var(--bg)' },
  { name: 'Associate',         amt: '₹30K', perks: 'standees · logo on creatives · 5 stories · stall', bg: 'var(--c2)', color: 'var(--ink)', amtColor: 'var(--ink)' },
  { name: 'Education Partner', amt: '₹25K', perks: 'standees · IG post · webinar promo · 25 reshares', bg: 'var(--c3)', color: 'var(--ink)', amtColor: 'var(--ink)' },
  { name: 'Cafe Partner',      amt: '₹20K', perks: 'standee + stall · reel · 45 reviews · 2K student reach', bg: 'var(--bg)', color: 'var(--ink)', amtColor: 'var(--ink)' },
  { name: 'Fashion Partner',   amt: '₹17.5K', perks: 'stall all days · reel · 2K student reach · 30 reviews', bg: 'var(--bg)', color: 'var(--ink)', amtColor: 'var(--ink)' },
]
  ...
  return (
    <section className="border-b-[1.5px] border-ink relative overflow-hidden pt-6 pb-7 sm:pt-14 sm:pb-16"
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 relative" style={{ zIndex: 2 }}>
        ...
        {/* Fix 15 */}
        <motion.h2 className="font-display leading-[0.9] text-balance mt-1.5 mb-2"
          style={{ fontSize: 'clamp(34px, 6.6vw, 76px)', letterSpacing: '-0.02em' }}
          initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ type: 'spring', stiffness: 220, damping: 24, delay: 0.08 }}>
          put your logo on something{' '}
          <span style={{ color: 'var(--c1)' }}>kids will actually remember.</span>
        </motion.h2>
        {/* Handwritten annotation */}
        <p className="font-hand mb-6" style={{ fontSize: 18, color: 'var(--ink)', opacity: 0.75 }}>
          ↳ that means stickers, stories + reels
        </p>

        {/* Fix 8: popIn with stagger(0.08), hover y-5 + boxShadow 7px */}
        <motion.div variants={stagger(0.08)} initial="hidden" whileInView="show"
          viewport={{ once: true, margin: '-40px' }}
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
          {TIERS.map((t) => (
            <motion.div key={t.name} variants={popIn}
              whileHover={{ y: -5, boxShadow: '7px 7px 0 var(--ink)' }} transition={SPRING}
              className="rounded-2xl border-[1.5px] border-ink p-4 sm:p-5 flex flex-col gap-2 min-h-[150px] sm:min-h-[170px]"
              style={{ background: t.bg, color: t.color, boxShadow: '3px 3px 0 var(--ink)', transitionProperty: 'transform, box-shadow' }}>
              <span className="font-mono text-[11px] tracking-[0.12em] uppercase opacity-88">{t.name}</span>
              <span className="font-display leading-[0.95]" style={{ fontSize: 'clamp(36px, 5vw, 52px)', color: t.amtColor, fontVariantNumeric: 'tabular-nums' }}>{t.amt}</span>
              <span className="font-body text-[13px] leading-relaxed opacity-90 mt-auto">{t.perks}</span>
            </motion.div>
          ))}
        </motion.div>
...
// ─── TICKETS ───────────────────────────────────────────────────────────────
const TICKETS_DATA = [
  { phase: 'phase 1 · early', price: '₹550', note1: 'opens now', note2: 'limited', bg: 'var(--c2)', color: 'var(--ink)' },
  { phase: 'phase 2 · regular', price: '₹650', note1: 'two weeks out', note2: 'most go here', bg: 'var(--c3)', color: 'var(--ink)' },
  { phase: 'spot · door', price: '₹700', note1: 'at the gate', note2: "if it's left", bg: 'var(--ink)', color: 'var(--bg)' },
]
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 grid gap-10 lg:grid-cols-2 lg:items-start relative" style={{ zIndex: 2 }}>
        <div>
          ...
          <motion.h2 className="font-display leading-[0.9] text-balance mt-1.5 mb-3"
            ...
            viewport={{ once: true }}
            transition={{ type: 'spring', stiffness: 220, damping: 24, delay: 0.08 }}>
            the only school night that ends at{' '}
            <span style={{ color: 'var(--c1)' }}>10pm.</span>
          </motion.h2>
          <p className="font-body leading-relaxed opacity-80 text-pretty" style={{ fontSize: 17, maxWidth: '42ch' }}>
            Friday, June 6 · 5pm – 10pm. Dance showcase, DJ, stalls, photo wall. Tickets get pricier as the date crawls closer — early birds win.
          </p>
        </div>
        ...
        <motion.div variants={stagger(0.07)} initial="hidden" whileInView="show"
          ...
          <MotionLink to="/paradox/afterparty"
            whileHover={{ x: -1, y: -1 }} whileTap={{ scale: 0.97 }} transition={SPRING}
...
// ─── GALLERY ───────────────────────────────────────────────────────────────
const GALLERY_ITEMS = [
  { rot: -3, label: 'P4.0 · finals', bg: 'var(--c3)' },
  { rot: 4,  label: 'after-party 2025', bg: 'var(--c1)', color: 'var(--bg)' },
  { rot: -2, label: 'MUN closing', bg: 'var(--c2)' },
  { rot: 5,  label: 'cricket finals', bg: 'var(--c3)', color: 'var(--bg)' },
  { rot: -4, label: 'shark tank pitch', bg: 'var(--c3)' },
  { rot: 3,  label: 'volunteers, 5am', bg: 'var(--ink)', color: 'var(--bg)' },
]
    ...
    <section className="border-b-[1.5px] border-ink relative overflow-hidden pt-6 pb-7 sm:pt-14 sm:pb-16"
      ...
      <div className="max-w-[1280px] mx-auto px-4 sm:px-8 lg:px-12 relative" style={{ zIndex: 2 }}>
        ...
        <motion.h2 className="font-display leading-[0.9] text-balance mt-1.5 mb-4 sm:mb-8"
          ...
          viewport={{ once: true }}
          transition={{ type: 'spring', stiffness: 220, damping: 24, delay: 0.08 }}>
          evidence we know how to{' '}
          <span style={{ color: 'var(--c1)' }}>throw a party.</span>
        </motion.h2>
          ...
          {GALLERY_ITEMS.map((item, i) => (
            <motion.figure key={i}
              ...
              <div className="aspect-square flex items-center justify-center"
                style={{
                  background: item.bg,
                  backgroundImage: 'repeating-linear-gradient(45deg, rgba(0,0,0,0.06) 0 8px, transparent 8px 16px)',
                }}>
                <span className="font-mono text-[10px] tracking-[0.1em] uppercase text-center opacity-75 px-2"
...
// ─── FAQ ───────────────────────────────────────────────────────────────────
const FAQS = [
  { q: 'who can register?', a: 'students grades 6–12 and college, across Kolkata. some events have age splits — check the event card.' },
  { q: 'do i need a team?', a: 'depends on the event. some are solo (Best Manager, FIFA, Photography), most are teams of 2–8. team size shows up next to each event.' },
  { q: 'where does the money go?', a: '100% of net proceeds fund AquaTerra welfare drives — teaching workshops, worksheets for underprivileged kids, food drives, plantation. 80G certificate available on request.' },
  { q: 'can my school sponsor or co-host?', a: 'yes. ping us — we have title, co-sponsor, associate and partner tiers. education partners get webinar slots too.' },
  { q: 'is the after-party included?', a: 'no — separate ticket (₹550 phase 1 → ₹700 spot). dance showcase 5–6pm, DJ till 10pm.' },
  { q: 'can i volunteer instead of competing?', a: 'absolutely. team aquaterra applications open on our IG. volunteers get certificates + welfare points.' },
]
...
const CONTACT_CARDS = [
  ...
  { lbl: 'vaibhav', val: '+91 97486 79979', href: 'tel:+919748679979', hoverBg: 'var(--c2)', hoverColor: 'var(--ink)' },
        ...
        <motion.div variants={stagger(0.07)} initial="hidden" whileInView="show"
          ...
          {CONTACT_CARDS.map((card, i) => (
            <motion.a key={card.lbl} href={card.href}
              ...
              variants={popIn}
              whileHover={{ y: -5, boxShadow: '5px 5px 0 var(--ink)' }} whileTap={{ scale: 0.97 }} transition={SPRING}
