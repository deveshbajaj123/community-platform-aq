...
const AQ_DATA: Record<string, AQNode> & { root: AQNode } = {
  ...
  ops: { id: 'ops', label: 'Core Departments', desc: 'The backbone: operations, content, finance, and HR keeping everything running.', stat: '10+ departments · 12 HoDs', category: 'ops', children: ['operations', 'content', 'finance', 'hr'] },
    ...
    >
      ...
      <div style={{
        ...
        <button onClick={onClose} style={{
          ...
          width: 40, height: 40, cursor: 'pointer', color: 'var(--ink-3)', fontSize: 16,
      ...
      <div style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 14, flex: 1 }}>
        <p style={{
          ...
          color: 'var(--ink-2)', margin: 0, textWrap: 'pretty' as any,
        }}>{node.desc}</p>
      ...
      {path.map((id, i) => {
        ...
        return (
          <span key={id} style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
            ...
            {!isLast && <span style={{ color: 'var(--ink-3)' }}>/</span>}
          </span>
        )
      })}
    </div>
...
function Legend() {
  return (
    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
      {LEGEND.map(it => (
        <div key={it.cat} style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--ink-3)' }}>
...
export default function AQNodeExplorer({ darkMode = false }: { darkMode?: boolean }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', width: '100%', height: '100%', background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--line)', overflow: 'hidden', display: 'flex' }}>
      {/* Dot grid */}
      ...
      <div style={{ flex: 1, position: 'relative' }}>
            ...
            <AnimatePresence initial={false}>
              {path.length > 1 && (
                <motion.button
                  ...
                  onClick={goBack}
                  style={{ background: 'var(--bg)', border: '1px solid var(--line)', borderRadius: 8, padding: '10px 14px', color: 'var(--ink)', fontFamily: 'var(--display)', fontSize: 12, fontWeight: 700, cursor: 'pointer', transition: 'background 0.1s', WebkitFontSmoothing: 'antialiased' }}
        ...
        {/* Hint */}
        {path.length === 1 && !panel && (
          <div style={{ position: 'absolute', bottom: 14, left: '50%', transform: 'translateX(-50%)', fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--ink-3)', letterSpacing: '0.06em', textTransform: 'uppercase', whiteSpace: 'nowrap', pointerEvents: 'none' }}>
